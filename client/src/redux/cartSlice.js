...
export const cartSlice = createSlice({
    ...
    reducers: {
        addProduct: (state, action) => {
            ...
            state.quantity = state.quantity + 1;
            state.total = state.total + (Number(action.payload.unit_price) * action.payload.quantity );
            guardar(state.products)
        },
        ...
        delProduct: (state, action) => {
            ...
            let filteredProducts = state.products.filter( p => p.id !== Number(action.payload))
...
export const { addProduct, delProduct, addItem, delItem, restoreCart, cleanCart } = cartSlice.actions;
